...
'更新：  v0.5  a [增加] 新增explorer崩溃事件id1000的触发
            '  b [修复] 修复一个processid=0的bug
            '  c [修复] 删除一个自动关闭的文件夹路径，忘了删除了
        ' v0.4  a [增加] windows 11 dev版快速访问(主文件夹)换了新的guid，本次更新增加了新的guid
        '      b [修复] 增加了taskkill，用来修复当用户不存在tskill时重启资源管理器失败的bug
            '  c [增加] 增加了后台检测资源管理器崩溃重启事件，只要触发重启事件，就自动打开上次的文件夹
            '  d [修复] 其他小优化修复
            '  e 改名为 restartexplorer

        ' v0.3  a [修复] 修复当ie窗口打开时使用脚本报错
